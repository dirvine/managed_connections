...
quick_error! {
    ...
    pub enum Error {
        ...
        InvalidState {}
        /// mio errors
        MioError(err: ::mio::NotifyError<::connection_handler::MioMessage>) {
            description("mio error")
            display("mio error: {}", err)
            cause(err)
            from()
        }
        ...
        /// Wrapper for a `secure::serialisation::SerialisationError`
        Serialisation(err: ::secure_serialisation::Error) {
            description("Serialisation error")
            display("Serialisation error: {:?}", err)
            // cause(err)
            from()
